...
--TASK 4.3
--All SQL Analysts including Senior SQL Analysts get a raise of 6%. Upate the salaries accordingly.

UPDATE employee
SET salary = salary * 6/100
...
SELECT COUNT(*)
FROM 
(SELECT emp_id, salary, ROUND(AVG(salary) OVER(PARTITION BY position_title),2) as avg_pos_sal FROM employee) a
...
SELECT *
FROM 
(SELECT  emp_id, salary,start_date, SUM(salary) OVER(ORDER BY start_date) as avg_pos_sal FROM employee) a
ORDER BY start_date;
...
--Task 10
--Create the same running total as the previous challenge but now also considder the fact that people were leaving the company.
...
- sum of salary,
- number of employees,
- average salary grouped by all meaningful combinations of
- division,
- department,
- position_title.
